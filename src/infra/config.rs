...
const ENV_GITHUB_OAUTH_CLIENT_ID: &str = "GITHUB_OAUTH_CLIENT_ID";
const ENV_GITHUB_OAUTH_CLIENT_SECRET: &str = "GITHUB_OAUTH_CLIENT_SECRET";
...
impl Config {
    pub fn load() -> Result<Config, Error> {
        ...
        let github_oauth = GithubOAuth::load()?;
        let http = Http::load()?;
        let jwt = Jwt::load()?;
...
impl Database {
    fn load() -> Result<Database, Error> {
        ...
        let connection_lifetime = std::env::var(ENV_DATABASE_CONNECTION_LIFETIME)
            .map_or(
...
impl GithubOAuth {
    fn load() -> Result<GithubOAuth, Error> {
        let client_id = std::env::var(ENV_GITHUB_OAUTH_CLIENT_ID).map_err(|_| {
            env_not_found(ENV_GITHUB_OAUTH_CLIENT_ID)
        })?;

        let client_secret = std::env::var(ENV_GITHUB_OAUTH_CLIENT_SECRET).map_err(|_| {
            env_not_found(ENV_GITHUB_OAUTH_CLIENT_SECRET)
        })?;

        let github_oauth = GithubOAuth { client_id, client_secret };
        Ok(github_oauth)
    }
}
...
impl Jwt {
    fn load() -> Result<Jwt, Error> {
        let access_token_secret = std::env::var(ENV_JWT_ACCESS_TOKEN_SECRET)
            .map_err(|_| env_not_found(ENV_JWT_ACCESS_TOKEN_SECRET))?;

        let refresh_token_secret = std::env::var(ENV_JWT_REFRESH_TOKEN_SECRET)
            .map_err(|_| env_not_found(ENV_JWT_REFRESH_TOKEN_SECRET))?;

        let access_token_duration = std::env::var(ENV_JWT_ACCESS_TOKEN_DURATION)
            .map_or(
        ...
        let refresh_token_duration = std::env::var(ENV_JWT_REFRESH_TOKEN_DURATION)
            .map_or(
...
impl Redis {
    fn load() -> Result<Redis, Error> {
        let url = std::env::var(ENV_REDIS_URL).map_err(|_| env_not_found(ENV_REDIS_URL))?;

        let pool_size = std::env::var(ENV_REDIS_POOL_SIZE)
            .map_or(Ok(DEFAULT_REDIS_POOL_SIZE), |pool_size_str| {
                pool_size_str.parse::<u32>()
            })?;

        let connection_lifetime = std::env::var(ENV_REDIS_CONNECTION_LIFETIME)
            .map_or(
